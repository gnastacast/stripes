...
void main()
{
   ...
   const float w = .5; // controls line width
   ...
   float u = 1./(1.+exp(s*(cos(f*( theta )) + cos(f*( phi   )) - w * 2.)));
   float v = 1;
   ...
   gl_FragColor.rgb = diffuse(N,L)*one*u*v + .5*specular(N,L,E)*one*u*v + .5*fresnel(N,E)*one;
   gl_FragColor.a = 1.;
}
